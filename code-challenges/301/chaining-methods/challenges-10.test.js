...
const count = (target, input) => {
  ...
  let result = arr.filter(num => target === num)
...
const totalSum = (input) => {
  input = input.reduce((acc, val) => acc.concat(val))
  return input.reduce((acc, val) => { return acc + val }, 0)
}
...
let findMaleAndFemale = (data) => {
  let ans = []
  data.forEach((el) => (el.gender === 'male' || el.gender === 'female') ? ans.push(el.name) : null)
  return ans.join(' and ')
}
