...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter(function (el) { return !forbiddenValues.includes(el) })
}
...
const getStatName = (arr, minBaseStat) => {
  arr = arr.filter(num => (num.baseStat > minBaseStat))
  let answer = []
  arr.forEach(obj => answer.push(obj.stat.name))
  return answer
};
...
const evenOddNumericValues = (arr) => {
  ...
  arr = arr.map(num => (num % 2 !== 1) ? 'even' : 'odd')
  return arr};
